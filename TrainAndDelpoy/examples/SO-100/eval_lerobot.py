...
class Gr00tRobotInferenceClient:
    ...
    def __init__(
        ...
        if len(robot_state_keys) == 12:  # dual arm
            self.modality_keys = ["left_arm", "left_gripper", "right_arm", "right_gripper"]
        else:  # single arm
            assert (
                len(robot_state_keys) == 6
            ), f"robot_state_keys should be size 6 for single arm, but got {len(robot_state_keys)} "
    ...
    def get_action(self, observation_dict, lang: str):
        ...
        # Convert to dual arm format for server
        if len(state) == 12:  # dual arm (bi_so101)
    ...
    def _convert_to_lerobot_action(
        ...
        # Convert action based on robot type
        if len(self.robot_state_keys) == 12:  # dual arm (bi_so101)
            # Server returns dual arm action: left_arm, left_gripper, right_arm, right_gripper
            assert len(concat_action) == len(self.robot_state_keys), f"this should be size {len(self.robot_state_keys)}"
