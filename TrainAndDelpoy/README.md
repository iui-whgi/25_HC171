...
```
ls /dev/ttyACM*로 각 포트 번호 확인 후, 원래 포트 맞는지 확인. 아닐 시에 다시 calibration 할듯
find_cameras.py를 통해 카메라 확인 
이때 usb허브별 카메라 한 대씩, 본체에 두 대 연결할 것

텔레오퍼레이션이 잘 되지 않을 때 -> 캘리브레이션 다시 할 것
팔로워 로봇 캘리브레이션 명령어
lerobot-calibrate \
...
허깅페이스 데이터셋에 올라가지 않았을 때 업로드 명령어
huggingface-cli upload JisooSong/sandwich_666to670_vhlv /home/son/lerobotjs/datasets/sandwich_666to670_vhlv --repo-type dataset

서버 열기
python scripts/inference_service.py --server --model-path JisooSong/full-sandwich-dataset-1 --embodiment-tag new_embodiment --data-config so101
